...
import net.runelite.osrsbb.internal.globvals.VarbitIndices;
import net.runelite.osrsbb.internal.globvals.VarpIndices;
import net.runelite.osrsbb.internal.globvals.VarpValues;
...
public class Combat extends MethodProvider {
    ...
    public boolean isAutoRetaliateEnabled() {
        return Vars.getVarp(VarpIndices.TOGGLE_AUTO_RETALIATE)
    ...
    public static boolean isAntifired() { return Vars.getBit(VarbitIndices.ANTIFIRE) > 0; }
    public static boolean isSuperAntifired() { return Vars.getBit(VarbitIndices.SUPER_ANTIFIRE) > 0; }
     ...
     */
    public boolean isPoisoned() { return 0 < Vars.getVarp(VarPlayer.POISON.getId()) && Vars.getVarp(VarPlayer.POISON.getId()) < 1000000; }
     ...
     */
    public boolean isEnvenomed() { return Vars.getVarp(VarPlayer.POISON.getId()) >= VENOM_THRESHOLD; }
    ...
    public boolean canBeFinished(final RSNPC npc) {
        ...
        switch (mobName) {
            case "Ancient Zygomite":
                isBelowOrAtHP(npc, getFinishableHP(150, 7));
                break;
            case "Gargoyle":
                isBelowOrAtHP(npc, getFinishableHP(105, 8));
                break;
            case "Zygomite":
                isBelowOrAtHP(npc, getFinishableHP(75, 7));
                break;
            case "Lizard":
                isBelowOrAtHP(npc, getFinishableHP(40, 4));
                break;
            case "Rockslug":
                isBelowOrAtHP(npc, getFinishableHP(27, 5));
                break;
            case "Desert Lizard":
                isBelowOrAtHP(npc, getFinishableHP(25, 4));
                break;
            case "Small Lizard":
                isBelowOrAtHP(npc, getFinishableHP(15, 4));
                break;
            default:
                throw new IllegalStateException("Unexpected value: " + mobName);
        }
        return false;
    }
}
