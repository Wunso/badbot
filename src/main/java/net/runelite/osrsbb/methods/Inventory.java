...
public class Inventory extends MethodProvider {
    ...
    public int checkIsSelected(RSWidgetItem[] comps) {
        ...
        { // This callback is on the game thread, move to executor thread
            {
                methods.badLite.getInjector().getInstance(ScheduledExecutorService.class).submit(()-> {
        ...
        methods.badLite.getInjector().getInstance(DrawManager.class).requestNextFrameListener(imageCallback);
        synchronized (selector) {
            try {
                selector.wait();
            } catch (Exception e) {
    ...
    public int getSelected(Image img, RSWidgetItem[] comps) {
        ...
        for (int c = 0; c < Math.min(comps.length, 28); ++c) {
            ...
            BufferedImage itemImage = methods.badLite.getItemManager().getImage(item.getID());
