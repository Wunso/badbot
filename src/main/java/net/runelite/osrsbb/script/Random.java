...
//	private int currentIndex = 0;
    ...
    public final void run(Script ctx) {
        ...
        ctx.ctx.badLite.getEventManager().removeListener(ctx);
        for (Script s : ctx.delegates) {
            ctx.ctx.badLite.getEventManager().removeListener(s);
        }
        ctx.ctx.badLite.getEventManager().addListener(this);
        ...
        ctx.ctx.badLite.getEventManager().removeListener(this);
        sleep(1000);
        ctx.ctx.badLite.getEventManager().addListener(ctx);
        for (Script s : ctx.delegates) {
            ctx.ctx.badLite.getEventManager().addListener(s);
        }
    }
