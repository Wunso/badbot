...
public abstract class Script extends Methods implements EventListener, Runnable {
    ...
    public final void delegateTo(Script script) {
        script.init(ctx);
        ctx.badLite.getEventManager().addListener(script);
    ...
    public final void run() {
        ...
        if (start) {
            running = true;
            ctx.badLite.getEventManager().addListener(this);
            log.info("Script started.");
            try {
                while (running) {
                    if (!paused) {
                        if (account.getName() != null && AccountManager.isTakingBreaks(account.getName())) {
                            BreakHandler h = ctx.badLite.getBreakHandler();
                            if (h.isBreaking()) {
        ...
        delegates.clear();
        ctx.badLite.getEventManager().removeListener(this);
        ctx.badLite.getScriptHandler().stopScript(id);
        id = -1;
    }

    private boolean checkForRandoms() {
        if (ctx.badLite.disableRandoms) {
            return false;
        }
        for (Random random : ctx.badLite.getScriptHandler().getRandoms()) {
            if (random.isEnabled() && !(ctx.badLite.disableAutoLogin && random instanceof LoginBot)) {
    ...
    private void blockEvents(boolean paint) {
        for (Script s : delegates) {
            ...
            if (paint && s instanceof PaintListener) {
                ctx.badLite.getEventManager().addListener(s, EventMulticaster.PAINT_EVENT);
            }
        }
        ctx.badLite.getEventManager().removeListener(this);
        if (paint && this instanceof PaintListener) {
            ctx.badLite.getEventManager().addListener(this, EventMulticaster.PAINT_EVENT);
        }
    }
    ...
    private void unblockEvents() {
        for (Script s : delegates) {
            ctx.badLite.getEventManager().removeListener(s);
            ctx.badLite.getEventManager().addListener(s);
        }
        ctx.badLite.getEventManager().removeListener(this);
        ctx.badLite.getEventManager().addListener(this);
    }

    public BadLite getBot() {
        return ctx.badLite;
    }
}
